...
image_file_dir = os.path.join(data_dir, 'earphones')
#print(image_file_dir)

xml_file_dir = os.path.join(data_dir, 'earphones')

annotations_info = {'images': [], 'annotations': [], 'categories': []}

categories_map = {'earphone': 1}
...
for i, file_name in enumerate(all_file_names):
    ...
    # image=cv2.imread('E:\\Python_workspace\\yolov5\\valid\\self\\valid\\582006.jpg')
    try:
        image = cv2.cvtColor(cv2.imread(image_file_path), cv2.COLOR_BGR2RGB)
    except:
        print(image_file_path)
    height, width, _ = image.shape
    image_info = {'file_name': image_file_name, 'id': file_name,
